...
4. **Submit a job with S3 test data**
   ...
   :::caution
   The Python SDK does not yet support the `test_data_ref` field. The `TestDataRef` and `S3TestDataRef` classes have not been implemented in `evalhub.models.api`. Use the **CLI** or **REST API** tab instead to submit jobs with custom S3 test data.
   :::
