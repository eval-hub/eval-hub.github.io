...
`--unfold` and `--unmask` are mutually exclusive unless used together — `--unmask` alone shows the raw path value without masking, while `--unfold` alone shows the file contents with sensitive values masked.
