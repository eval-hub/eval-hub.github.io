...
```bash
pip install "eval-hub-sdk[server,cli]>=1.0.3"
```

For a basic evaluation, start the server with its built-in local defaults (available since eval-hub-sdk 1.0.3):

```bash
evalhub server start
```

If you need to customize server settings, create a configuration file (for
example, `my-config.yaml`). Settings you might customize include
`service.port` or `mlflow.tracking_uri`, among others:
...
Starting with eval-hub-sdk 1.0.3, Local mode starts a sidecar automatically. The server runs evaluation jobs as subprocesses in local mode. Authentication is disabled and CORS is enabled automatically.
...
For full experiment schema, adapter backends, and cluster sidecar proxy behavior, see the [MLflow guide](/guides/mlflow/).
:::
...
| **Multi-tenancy** | Single-tenant or multi-tenant (`X-Tenant` header) | Single-tenant only |
| **CORS** | Disabled by default | Enabled |
| **Sidecar proxy** | Injected into job pods | Started automatically |
...
<Since version="1.0.3 eval-hub-sdk" />

Local mode also supports remote OpenAI-compatible model endpoints, including HTTPS endpoints that require authentication or a custom CA certificate. Use the `auth.secret_ref` field in the evaluation job:
...
model:
  ...
  auth:
    secret_ref: file:///home/user1/model-auth
```

In Local mode, `file:///...` identifies a directory on the local filesystem. Add only the files required by the model endpoint:

| File | Purpose |
|---|---|
| `api-key` | Authenticates requests to the model server |
| `ca_cert` | Provides a custom CA certificate for the HTTPS endpoint |
| `hf-token` | Provides a Hugging Face access token |

The `auth` section and each file are optional. Protect the directory and its contents with appropriate filesystem permissions, and do not commit credentials to source control.

For model authentication patterns, see [Writing an adapter](/guides/model-authentication/#writing-an-adapter) in the Model Authentication guide. The adapter can use the same authentication approach in both modes: in Local mode, `model.auth.secret_ref` points to a local `file:///...` directory containing the required files; in cluster mode, it refers to the Kubernetes secret name.
...
| MLflow experiment not created | MLflow not configured | Set `mlflow.tracking_uri` in the server config YAML (`server_config_file`); see [MLflow](/guides/mlflow/) |
