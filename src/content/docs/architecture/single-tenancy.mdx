...
1. **Deploy EvalHub**
   ...
   | `evalhub-tenant-admin` Role | Full CRUD on virtual EvalHub resources + MLflow read |
   | `evalhub-user` Role | Read providers/collections, submit via `evalhubs/proxy`, create status-events |
   | `evalhub-tenant-admin-binding` | Binds admin Role to `system:serviceaccounts:team-a` |

   :::note[Default admin access]
   The operator binds **all** ServiceAccounts in the namespace to `evalhub-tenant-admin`. For restricted access, create an explicit RoleBinding to `evalhub-user` instead.
   :::
