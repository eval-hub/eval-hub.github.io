...
3. **Connect to EvalHub**
   ...
   The commands below assume an EvalHub service is reachable. On a cluster, use
   the route or service URL. For local development, start the server first:
...
When MLflow is configured on the EvalHub server, pass an experiment name to track the job. See the [MLflow guide](/guides/mlflow/) for tags, artifact location, adapter backends, and cluster vs local wiring.
...
job = client.jobs.submit(JobSubmissionRequest(
    ...
    experiment=ExperimentConfig(
        name="my-experiment",
        tags=[ExperimentTag(key="environment", value="testing")],
    ),
))
```
